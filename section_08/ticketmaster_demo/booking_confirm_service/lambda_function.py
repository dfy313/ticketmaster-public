...
def lambda_handler(event, context):
    ...
    # 4. Clean up redis
    r.delete(lock_key)
    if ticket_id.endswith("H"): # Publish if this is a high-traffic event (e.g., ends with 'H')
        r.publish("ticket_updates", json.dumps({
            "ticketId": ticket_id,
            "price": price,
            "userId": USER_ID,
            "status": "confirmed"
    ...
    html_content = (
        f"<html><body><h2>Booking Confirmed!</h2>"
        f"<p>Ticket: {ticket_id}</p><p>Price: ${float(price):.2f}</p><p>User: {USER_ID}</p><p>Payment Intent: {intent.id}</p>"
