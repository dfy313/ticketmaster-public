...
SEARCH_CDN_URL = "<YOUR_SEARCH_CDN_URL>" # Remeber to include https:// prefix
...
def handle_home_request():
    ...
    html_content += (
        f"<form onsubmit=\"event.preventDefault();"
        f" window.location.href='{SEARCH_CDN_URL}/search/' + "
        f"encodeURIComponent(document.getElementById('searchInput').value);\">"
        f"<input type='text' id='searchInput' placeholder='Search events...' required></form>"
    )
    for event in events:
